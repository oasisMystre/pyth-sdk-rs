...
fn main() {
    ...
    let eth_price_key = Address::from_str(key).unwrap();

    loop {
        // get price data from key
        let eth_price_account = clnt.get_account(&eth_price_key).unwrap();
        let eth_price_feed =
            SolanaPriceAccount::account_to_feed(&eth_price_key, &eth_price_account).unwrap();
